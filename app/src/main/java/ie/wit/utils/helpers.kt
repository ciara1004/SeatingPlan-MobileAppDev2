...
fun createLoader(activity: FragmentActivity) : AlertDialog {
    ...
    loader.setIcon(R.mipmap.ic_launcher_logo_round)
...
fun uploadImageView(app: SeatingPlanApp, imageView: ImageView) {
    ...
    uploadTask.addOnFailureListener { object : OnFailureListener {
        override fun onFailure(error: Exception) {
            Log.v("Seating Plan", "uploadTask.exception" + error)
        }
    }
...
fun updateAllTables(app: SeatingPlanApp) {
    val userId = app.auth.currentUser!!.uid
    val userEmail = app.auth.currentUser!!.email
    var tableRef = app.database.ref.child("tables")
                                  .orderByChild("email")
    val usertableRef = app.database.ref.child("user-tables")
                                  .child(userId).orderByChild("uid")

    tableRef.equalTo(userEmail).addListenerForSingleValueEvent(
    ...
    usertableRef.addListenerForSingleValueEvent(
        object : ValueEventListener {
        override fun onCancelled(error: DatabaseError) {}
        override fun onDataChange(snapshot: DataSnapshot) {
            snapshot.children.forEach {
...
fun writeImageRef(app: SeatingPlanApp, imageRef: String) {
    val userId = app.auth.currentUser!!.uid
...
fun validatePhoto(app: SeatingPlanApp, activity: Activity) {
...
fun checkExistingPhoto(app: SeatingPlanApp, activity: Activity) {

    app.userImage = "".toUri()
    Log.v("Seating Plan","checkExistingPhoto 1 app.userImage : ${app.userImage}")
    ...
    app.database.child("user-photos").orderByChild("uid")
        ...
        override fun onDataChange(snapshot: DataSnapshot ) {
            snapshot.children.forEach {
                ...
                Log.v("Seating Plan","checkExistingPhoto 2 app.userImage : ${app.userImage}")
            }
            Log.v("Seating Plan","validatePhoto 3 app.userImage : ${app.userImage}")
